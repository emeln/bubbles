...
class SigmaInterpolator(object):
    ...
    def __sigma_sq_interpolation(self, r, z):
        return np.squeeze(self.__interpolator.ev(np.log10(r), z)) # Evaluates sigma^2 at r, z

    def __dsigma_sq_dr_interpolation(self, r, z): # Evaluates dsigma^2/dr at r, z
        return np.squeeze(self.__interpolator_dr.ev(np.log10(r), z))

    def __dsigma_sq_dloga_interpolation(self, r, z): # Evaluates dsigma^2/da at r, z (?) Not sure if a is normal a or something else
        return np.squeeze(self.__interpolator_dloga.ev(np.log10(r), z))

    def __call__(self, m, z): # Calling sigma_interpolation with m, z returns sigma(m,z)
        return np.sqrt(self.__sigma_sq_interpolation(self.radius_of_mass(m), z)) # radius_of_mass(m) comes from
        # WindowFunctions.set_window_functions in __init__

    def sigma_of_r(self, r, z): # Returns sigma(r,z)
    ...
    def dsigma_dm(self, m, z):
        # This one depends on using the top hat window function & that the density is constant rho_mean I think
        # Basically it depends on r(m) proportional to m^(1/3)
        # Which arises from m = rho * V(r) = rho * (4/3) * pi * r^3 ~ r^3
